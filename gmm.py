...
# -------------------------------------------------------
# Configuration for datasets (file path, random seeds, etc.)
# -------------------------------------------------------
file_cfgs = {
    "Dataset A": {"path": "results/dataset_a/case_as.json", "random_state": 42, "init": "random"},
    "Dataset B": {"path": "results/dataset_b/case_as.json", "random_state": 42, "init": "random"},
    "Dataset C": {"path": "results/dataset_c/case_as.json", "random_state": 42, "init": "random"},
    "Dataset D": {"path": "results/dataset_d/case_as.json", "random_state": 42, "init": "random"},
}

# Stage index to name mapping
stage_names = {0: "None", 1: "Mild", 2: "Moderate", 3: "Severe"}
classes = sorted(stage_names)

# Grid of false positive rates for ROC interpolation
...
for name, cfg in file_cfgs.items():
    ...
    with open(cfg["path"], "r") as f:
        data = json.load(f)
    df = pd.DataFrame(
        [{"id": k, "pred_as": v["pred_as"], "stage": v["stage"]} for k, v in data.items()]
    )

    # Fit Gaussian Mixture Model (4 components = 4 stages)
    gmm = GaussianMixture(n_components=4, random_state=cfg["random_state"], init_params=cfg["init"])
    df["cluster"] = gmm.fit_predict(df[["pred_as"]])

    # Map clusters to stage labels using majority vote
    mapping = {
    ...
    # -------------------------------------------------------
    # Beeswarm plot (distribution of predictions per stage)
    # -------------------------------------------------------
    ...
    df_plot = pd.DataFrame({"pred_as": pred_vals, "stage_idx": df["mapped"]})
    ...
    # Draw swarmplot with median + IQR overlay
    ...
    # Overlay median (diamond) and IQR (horizontal line)
    ...
    # Custom legend
    handles = [
        mpatches.Patch(color=stage_palette[st], label=st)
        for st in stage_order
        if st in df_plot["stage_name"].unique()
    ]
    ...
    # -------------------------------------------------------
    # Align predicted probabilities to stage mapping
    # -------------------------------------------------------
    probs_raw = gmm.predict_proba(df[["pred_as"]])  # (n_samples, 4)
    ...
    # Stratify indices by stage for bootstrap resampling
    strata_idx = {st: df[df["stage"] == st].index.to_numpy() for st in classes}
    tprs_micro, tprs_macro, metric_rows = [], [], []

    # -------------------------------------------------------
    # Bootstrap loop (compute CI for metrics and ROC curves)
    # -------------------------------------------------------
    for _ in tqdm(range(n_bootstrap), desc=f"{name} bootstrap", leave=False):
        # Resample within each stage to maintain class balance
        idxs = np.concatenate(
            [resample(strata_idx[st], replace=True, n_samples=len(strata_idx[st])) for st in classes]
        )
        y_t, y_p = y_true[idxs], y_pred[idxs]
        probs_bs = probs_aligned[idxs]
        y_bin = label_binarize(y_t, classes=classes)

        # Collect metrics for this bootstrap replicate
        metric_rows.append(
            {
                "auc_micro": roc_auc_score(pd.get_dummies(y_t), probs_bs, average="micro", multi_class="ovr"),
                "auc_macro": roc_auc_score(pd.get_dummies(y_t), probs_bs, average="macro", multi_class="ovr"),
        ...
        # Compute ROC (macro = average across classes)
        all_fpr = np.unique(np.concatenate([roc_curve(y_bin[:, i], probs_bs[:, i])[0] for i in range(4)]))
        mean_tpr = (
            sum(np.interp(all_fpr, roc_curve(y_bin[:, i], probs_bs[:, i])[0], roc_curve(y_bin[:, i], probs_bs[:, i])[1])
                for i in range(4)) / 4
        )
...
# -------------------------------------------------------
# Function: plot ROC curves for all datasets (family plots)
# -------------------------------------------------------
...
def plot_family(mean_key: str, auc_key: str, outfile: str):
    """Plot ROC curves for all datasets with mean and AUC labels."""
    ...
    plt.plot([0, 1], [0, 1], "--", color="gray", linewidth=1)  # diagonal line
    plt.xlim(0, 1)
    plt.ylim(0, 1.05)
...
# Plot ROC family curves (micro + macro)
